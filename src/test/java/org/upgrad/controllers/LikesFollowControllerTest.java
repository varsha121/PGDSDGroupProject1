...
@WebMvcTest(LikeFollowController.class)
  ...
  @MockBean
   ...
   private AnswerService answerService;
    ...
    public void addLikesWithLogin() throws Exception{
        ...
        String answerId ="1";
        given(likeService.getLikes(user.getId())).willReturn(null);
        String url = "/api/like/1";
        mvc.perform(post(url).session(session)
                .param("answerId",answerId))
    ...
    public void addAlreadyLikedLikesWithLogin() throws Exception{
        ...
        String answerId ="1";
        Like like = new Like();
        given(likeService.getLikes(user.getId())).willReturn( like);
        String url = "/api/like/1";
        mvc.perform(post(url).session(session)
                .param("answerId",answerId))
                .andExpect(status().is4xxClientError())
    ...
    public void removeLikesWithoutAuth() throws Exception{
       ...
       user.setRole("user");
        ...
        String answerId ="1";
        user.setId(3);
        Mockito.when(likeService.getUserId(1)).thenReturn(null);
        String url = "/api/unlike/1";
        mvc.perform(delete(url).session(session)
                .param("answerId",answerId))
                .andExpect(status().is4xxClientError())
   ...
   @Test
    ...
    public void removeFollowWithoutFollows() throws Exception{
        ...
        String categoryId ="1";
        user.setId(3);
        Mockito.when(followService.findUserId(1)).thenReturn(null);
        String url = "/api/unfollow/1";
        mvc.perform(delete(url).session(session)
                .param("categoryId",categoryId))
                .andExpect(status().is4xxClientError())
    ...
    public void removeFollowWithAuth() throws Exception{
        ...
        follow.setId(1);
        Mockito.when(followService.findUserId(1)).thenReturn(3);
        String url = "/api/unfollow/1";
        mvc.perform(delete(url).session(session)
                .param("categoryId",categoryId))
               .andExpect(status().is2xxSuccessful())
