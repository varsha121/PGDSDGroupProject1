   ...
   protected MockHttpSession session;
    ...
    public void editCommentWithAuthorization() throws Exception{
        ...
        mvc.perform(put(url).session(session)
                ...
                .param("commentId",commentId))
               .andExpect(status().is2xxSuccessful())
                .andExpect(content().string(Matchers.containsString(" CommentRepository with commentId " +commentId+ " edited successfully.")));
    }
    ...
    public void deleteCommentWithAdminAuthorization() throws Exception{
       ...
       String url = "/api/comment/1";
        mvc.perform(delete(url).session(session)
                .param("commentId",commentId))
                .andExpect(status().is2xxSuccessful())
                .andExpect(content().string(Matchers.containsString(" CommentRepository with commentId " +commentId+ " deleted successfully.")));
    }
