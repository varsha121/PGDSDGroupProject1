...
public class AnswerControllerTest {
    ...
    public void createAnswerWithAuthentication() throws Exception {
        ...
        mvc.perform(post(url).session(session)
                ...
                .andExpect(content().string(containsString("AnswerService to questionId 3 added successfully")));
    }
    @Test
    ...
    public void editAnswerWithAuthorization() throws Exception {
        ...
        mvc.perform(put(url).session(session)
                .param("answer",answer))
                .andExpect(status().is2xxSuccessful())
                .andExpect(content().string(Matchers.containsString(" AnswerService with answerId 3 edited successfully.")));
    }
   ...
   @Test
    public void deleteAnswerWithAuthorization() throws Exception {
        ...
        mvc.perform(delete(url).session(session))
                .andExpect(status().is2xxSuccessful())
                .andExpect(content().string(Matchers.containsString(" AnswerService with answerId 3 deleted successfully.")));
    }
}